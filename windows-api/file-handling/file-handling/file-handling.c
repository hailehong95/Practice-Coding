...
void MyReadFile(TCHAR* filePath, char* dataBuffer, DWORD dwBytesToRead)
{
	...
	if (hFileReader == INVALID_HANDLE_VALUE) {
		printf(" [-] Terminal failure: Unable to open file \"%ls\" for read. Error Code = %d\n", filePath, GetLastError());
		return;
	}

	printf(" [+] Reading %d bytes from \"%ls\".\n", dwBytesToRead, filePath);
...
int main()
{
	...
	wsprintfW(filePath, TEXT("%s\\dummy-text.txt"), filePath);
